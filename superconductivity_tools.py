...
from anharmonicity import calculate_anharmonicity_from_E_C_and_E_J_fifth_order
...
def calculate_f01_from_RT_resistance_and_anharmonicity(
    room_temperature_resistance,
    anharmonicity_Hz,
    Delta_cold_eV,
    difference_between_RT_and_cold_resistance = 1.1385,
    ...
    ''' Given a room temperature resistance, calculate the resulting f_01.
        ...
        E_C is the transmon's charging energy.
            ...
            with some condition specifying what the anharmonicity is.
        Delta_cold is the superconducting gap at millikelvin temperatures.
        T is the temperature of operation, typically dilution fridge
          temperatures. For instance, 10 mK.
    '''
    ...
    ## Now, calculate what E_C must be, given some known anharmonicity.
    def find_E_C_given_anharmonicity_and_E_J(
        known_anharmonicity_Hz,
        known_E_J_Hz,
        tolerance_Hz = 100
        ):
        
        # Initialise values.
        ## First guess for E_C is just the anharmonicity.
        ## The target value is expected to be about 10% lower.
        E_C_iterator_Hz = -1 * known_anharmonicity_Hz
        ## Some massive initial guess thing just to start the loop.
        resulting_anharmonicity_Hz = -500e6
        
        # Set max limit.
        max_limit_counter = 1000000
        
        while (( np.abs(resulting_anharmonicity_Hz - known_anharmonicity_Hz) > tolerance_Hz ) and (max_limit_counter > 0)):
            # Update the iterator.
            if ( resulting_anharmonicity_Hz < known_anharmonicity_Hz ):
                # The found anharmonicity is more negative than the known anharmonicity.
                ## So, lower the E_C guess.
                E_C_iterator_Hz *= 0.997
            else:
                # The found anharmonicity is less negative than the known anharmonicity.
                ## So, increase the E_C guess carefully.
                E_C_iterator_Hz += tolerance_Hz
            
            # Try a new value.
            resulting_anharmonicity_Hz = calculate_anharmonicity_from_E_C_and_E_J_fifth_order(
                E_C_Hz = E_C_iterator_Hz,
                E_J_Hz = known_E_J_Hz
            )
        ...
        # Failed attempt?
        if max_limit_counter <= 0:
            raise RuntimeError("Halted! Could not find a legal E_C - Counter timeout. The E_C at the time of timeout was: "+str(E_C_iterator_Hz/1e6)+" MHz")
        
        # At this point, there is a known E_C!
        return E_C_iterator_Hz
    
    # Iterate until a legal E_C is found.
    E_C = h * (find_E_C_given_anharmonicity_and_E_J( anharmonicity_Hz, (E_J/h) ))
...
def compare_aging_vs_junction_sizes(
    ...
    ## MT's provided Junction size and error bar data.
    ## TODO: Import from files instead.
    thin_dim  = [300, 550, 250, 500, 200, 450, 150, 400, 100, 350]
    thick_dim = [600, 550, 500, 450, 400, 350, 300, 250, 200, 150]
    thin25 = [[1.1670094451247615, 1.2021286319505107, 1.1759430034160094, 1.1509973343425313, 1.178893024588443, 1.1590337794602752, 1.1685235541733467, 1.1820631389222347, 1.2649834129638389, 1.1866183983320386, 1.184231271900367, 1.1380000601266025, 1.249169307808557, 1.1617409537508128], [1.0447454223031818, 1.1005071504214043, 1.140944379356104, 1.098723165277058, 1.1634520437830762, 1.0518471649873156, 1.08518682515137, 1.1735588528587548, 1.0509496523899584], [1.206535233354763,
, 1.0192568849663652, 1.0209670707416951, 1.022035680000492, 1.0239764165595397], [1.0277114811090544, 1.024413573907201, 1.0297611767280637], [1.033353847008972, 1.0274228429878738, 1.0245788985534916, 1.030044896521741, 1.0252917055142752, 1.0289844562159363, 1.02532277259394, 1.0256235479377225, 1.027457656326007], [1.031837455089609, 1.0305020848663697, 1.0311753626027462, 1.0300441199967887, 1.0295496487477591, 1.0284230101836345, 1.0263130769567523, 1.026981837329484, 1.02873306510947, 1.0309087180602252], [1.0335359779274107, 1.0277940304800353, 1.0396429920404597, 1.0363561738526506, 1.0301174972529947, 1.0350189994691121, 1.0318941328635765]]
    def to_percent(data):
        return [[(x - 1) * 100 for x in sublist] for sublist in data]
    thin25   = to_percent(thin25)
    thin50   = to_percent(thin50)
    thin100  = to_percent(thin100)
    thin1    = to_percent(thin1)
    thin10   = to_percent(thin10)
    thick25  = to_percent(thick25)
    thick50  = to_percent(thick50)
    thick100 = to_percent(thick100)
    thick1   = to_percent(thick1)
    thin_tot  = [ thin25,  thin50,  thin100,  thin1, thin10]
    thick_tot = [thick25, thick50, thick100, thick1        ]
    
    ## Now that MT's data is imported, compute means and standard deviations.
    def compute_means_stds(datasets, n_dim):
        all_values_per_dim = [[] for _ in range(n_dim)]
        for data in datasets:
            for i, values in enumerate(data):
                if i < n_dim:
                    all_values_per_dim[i].extend(values)
        means = [np.mean(v) if v else np.nan for v in all_values_per_dim]
        stdevs = [np.std(v) if v else np.nan for v in all_values_per_dim]
        return means, stdevs
    n_dim_thin  = len(thin_dim)
    n_dim_thick = len(thick_dim)
    means_thin,  stdevs_thin  = compute_means_stds(thin_tot,  n_dim_thin)
    means_thick, stdevs_thick = compute_means_stds(thick_tot, n_dim_thick)
    
    ## At this point, the means_thin and means_thick lists are weirdly sorted.
    ## So, they should be sorted, based on thin_x and thick_x.
    thin_combined = list(zip(thin_dim, means_thin, stdevs_thin))
    thin_combined_sorted = sorted(thin_combined, key=lambda x: x[0])
    thin_dim_sorted, means_thin_sorted, stdevs_thin_sorted = map(list, zip(*thin_combined_sorted))
    thick_combined = list(zip(thick_dim, means_thick, stdevs_thick))
    thick_combined_sorted = sorted(thick_combined, key=lambda x: x[0])
    thick_dim_sorted, means_thick_sorted, stdevs_thick_sorted = map(list, zip(*thick_combined_sorted))
    ...
    ax1.errorbar(
        thin_dim_sorted, means_thin_sorted,
        yerr=stdevs_thin_sorted,
        fmt='o', ms=14, capsize=5, lw=2,
        color="#1C70EE", label="Aging, Low-dose"
    )
    ax1.errorbar(
        thick_dim_sorted, means_thick_sorted,
        yerr=stdevs_thick_sorted,
        fmt='o', ms=14, capsize=5, lw=2,
        color="#EE1C1C", label="Aging, Medium-dose"
    )
    ax1.set_xlim(-25, 675)
    ax1.set_ylim(-1, 42)
    ax1.tick_params(axis='both', labelsize=26)
